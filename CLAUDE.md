...
- `/etc/X11/xorg.conf.d/10-gt730-only.conf` — pins Xorg to the GT 730 (`Driver "modesetting"`, `BusID "PCI:4:0:0"`, `kmsdev=/dev/dri/card0`) and provides a Screen + ServerLayout. Without this, Xorg picks the P100 first by PCI order and dies with `(EE) [drm] Failed to open DRM device for (null): -2` / `no screens found` because `nvidia-drm modeset=0` leaves the P100 with no KMS. Use `modesetting`, not the legacy UMS `nouveau` Xorg driver (the latter trips on `xf86EnableIO: failed to enable I/O ports`).
